...
class AirtableScraper:
    ...
    def _get_table_data(self) -> list:
        ...
        self.__type_handlers = {
            ...
            "date": lambda cell_val: '"' + cast_to_str(cell_val, sep=",", tz=self.timezone, date_only=True) + '"',
            ...
            "lookup": lambda cell_val: '"' + join_list_like(flatten_lookup_column_r(list(cell_val["valuesByForeignRowId"].values()), type_options=col_def.typeOptions, flat_list=[]), ",") + '"',  # variable output from looking up columns from other table
            "multilineText": lambda cell_val: f'"{cell_val}"',
            "multipleAttachment": lambda cell_val: f'"{",".join([f["filename"] for f in cell_val])}"',
            "multiSelect": lambda cell_val: '"' + ",".join([col_def.typeOptions.get(val) for val in cell_val]) + '"',
            "number": lambda cell_val: cell_val,
            "phone": lambda cell_val: cell_val,
            "rating": lambda cell_val: cell_val,
            "richText": lambda cell_val: '"' + "".join([section["insert"] for section in cell_val.get("documentValue")]).strip() + '"',
